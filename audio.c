/*
 *
 * This .c file contains all of the functions for reading and playing audio
 * Most of the contents of this file were taken from Harald Rosenfeldt's blog
 * and was mentioned in Chris's tips document.
 *
 * Link: www.harald-rosenfeldt.de/2017/12/31/zynq-read-a-wav-file-from-sd-card-and-play-it-on-the-audio-codec/
 *
 * My contributes mainly involve the very simple task of converting this from a standalone program
 * into functions other .c files can call.
 * more functionality was intended but not completed
 *
 * My functions:
 *
 * void init_audio()
    ...
    if (theBuffer) {
    	...
    	// Changing the volume and swap left/right channel and polarity
    	{
    		...
    		for(u32 i=0;i<theBufferSize/4;++i) {
    			...
    			int left_i  = -(int)left * theVolume / 10;			//volume calculations were changed slightly
    			int right_i = -(int)right * theVolume / 10;			//to fit our needs
...
//Clean up audio
//Clear the DMA to stop audio playback and to also prepare for new audio, or just to clean up at the end of usage
void cleanup_audio(){
	adau1761_dmaReset(&codec);
	return;
}

//Take in external values of num to represent the song index
//and vol to represent the volume
void remotePlay(int num, int vol){
	theVolume = vol; 	//set internal volume value to external value being tracked by game logic
	stopWavFile();		//stop any currently playing audio
	playWavFile(files[num]);	//play the requested audio
	return;
}

//Initialize everything needed to play audio from an SD card
void init_audio()
{
